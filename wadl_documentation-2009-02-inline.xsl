...
-->
    ...
    <xsl:template match="wadl:response">
        ...
        <!-- mru1 -->
        <li>
        <h5>Reponse <xsl:value-of select="@status"/> [<xsl:value-of select="wadl:doc[@title][1]/@title"/>]</h5>        
        <xsl:apply-templates select="wadl:doc"/>
            
        <xsl:if test="wadl:representation">
            <p><em>available response representations:</em></p>
            <ul>
                <xsl:apply-templates select="wadl:representation" mode="inline"/>
            </ul>
        </xsl:if>
    ...
    <xsl:template match="wadl:representation|wadl:fault" mode="inline">
        <xsl:variable name="id"><xsl:call-template name="get-id"/></xsl:variable>
        <xsl:variable name="href" select="@id"/>
        <xsl:variable name="expanded-name">
            <xsl:call-template name="expand-qname">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <li>
                    <xsl:call-template name="representation-name2"/>
    ...
    <xsl:template name="representation-name2">
        <xsl:variable name="expanded-name">
            <xsl:call-template name="expand-qname">
                <xsl:with-param select="@element" name="qname"/>
            </xsl:call-template>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="@status"></xsl:if><xsl:value-of select="@status"/>
                [<xsl:value-of select="@mediaType"/>]
                <xsl:if test="@element"> (</xsl:if>
                <abbr title="{$expanded-name}"><xsl:value-of select="@element"/></abbr>
                <xsl:if test="@element">)</xsl:if>
